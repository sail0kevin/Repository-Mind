...
class CodeGraphBuilder:
    ...
    def build_from_directory(self, repo_path, scanned_files: list[dict] | None = None) -> CodeGraph:
        """复用仓库扫描器的忽略和可索引结果构建 Python 图谱。"""
        root = Path(repo_path)
        records = scanned_files or []
        discovered_sources = [
            Path(item["absolute_path"])
            for item in records
            if item.get("ignored_reason") is None and item.get("absolute_path")
        ]
        python_sources = [path for path in discovered_sources if path.suffix.lower() == ".py"]
        diagnostics: dict = {
    ...
    def parse_file(self, graph: CodeGraph, python_file: Path, root: Path) -> None:
        ...
        # M1 只保存可证实的定义节点；真实跨文件调用关系留到 M2 AST 解析实现。
        return None
