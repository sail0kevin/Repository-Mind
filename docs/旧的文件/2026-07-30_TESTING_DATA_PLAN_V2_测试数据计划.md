...
> **具体纠正（2026-08-05）**：本文档「一、压力面问题全景图」将检索延迟标注为「❌ 完全缺失」，该判断错误。
> `retrieval_runs.duration_ms` 一直记录延迟数据：683 次检索 p50=7.1ms / p95=30.3ms / max=55.3ms（纯 BM25 + SQLite FTS5，检索阶段耗时，不含证据组装）。
> 此外本文档引用的冻结集指标未标注「纯词法模式」，引用时须补充该限定。
