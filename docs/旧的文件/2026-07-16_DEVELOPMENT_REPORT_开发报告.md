...
> 归档说明：这是 2026-07-16 的阶段报告。当前实现状态、验收指标和执行顺序以 `docs/后续开发指导/2026-08-01_IMPROVEMENT_PLAN_V2_当前改进执行计划.md` 为准；本文仅用于追溯历史。
...
- Legacy 协作移动到高级入口。
- 后端测试：122 passed（60 warnings；本地当前工作区验证）
- 桌面测试：53 passed（6 个测试文件；本地当前工作区验证）
...
- Electron 验证后端：实例 ID、API v1、Contract、Schema、稳定数据库身份指纹和 Session ID。
- 并发启动共享单个 in-flight Promise；旧 ChildProcess 的 exit/error 事件不能清理新会话。
- userData 身份固定为 `%APPDATA%\repomind-desktop`。
- 应用身份保持：
  - package name：`repomind-desktop`
  - appId：`com.repomind.app`
  - productName：`RepoMind`
- 关闭时只进行一次带会话令牌的正常终止尝试；若后端未及时退出，Electron 不强杀或扫描进程。
- Electron 在启动 packaged/dev 后端时注入自己的 PID；Windows 后端只在该 PID 等于启动瞬间直接父进程后，以 `OpenProcess(SYNCHRONIZE)` 获取 HANDLE 并用 `WaitForSingleObject` 等待。正常认证关闭仍是首选路径；若其超时或 Electron 崩溃，HANDLE 会变为 signaled，后端监视线程随后让后端自行退出。
- 非 Windows 不进行可能受 PID 复用影响的轮询；未注入父 PID 的独立后端开发与测试保持原行为。
...
`.github/workflows/ci-windows.yml` 现在会响应 `public-main`、`main`、`master` 的 push、pull request 和手动触发；远端是否通过仍必须查看实际 GitHub Actions 运行记录。当前不宣称远端 CI 全绿、签名安装包或已发布 GitHub Release。

### Windows 下载、安装、Portable、校验与卸载

仓库包含 Windows 打包和 Release workflow，但当前不代表 GitHub Releases 已经存在可下载版本。维护者实际创建 Release 后，用户应从该仓库的 **Releases** 页面下载对应版本的 Setup、Portable 和 `SHA256SUMS.txt`，并用 PowerShell `Get-FileHash -Algorithm SHA256` 对照清单校验。Setup 可通过 Windows“设置 → 应用 → 已安装的应用”卸载；Portable 退出后删除 EXE 即可。两者默认都把用户数据放在 `%APPDATA%\repomind-desktop`，删除程序不会自动删除该目录；需要清理数据时应先退出程序，再手动删除。

当前构建没有代码签名证书，SmartScreen 可能提示未知发布者。只有在确认文件来自本仓库 Releases 页面且 SHA-256 一致后，才可在 SmartScreen 中选择“更多信息”并决定“仍要运行”；不应把绕过提示当作签名或安全认证。


### 历史本地构建产物记录（不是当前 GitHub Release 附件）

以下文件名和哈希只记录此前在本机生成的历史构建产物。它们没有提交到仓库，也不表示当前 GitHub Releases 页面存在可下载附件；用户只能信任实际 Release 页面同一版本提供的文件及其随附 `SHA256SUMS.txt`。
...
本机 Node 为 24/npm 11，因此 `npm ci --ignore-scripts` 报告了与项目要求 Node 20/npm 10 不一致的 `EBADENGINE` 警告，但完成安装；当前工作区桌面测试为 53 passed，且构建完成。正式 CI 固定 Node 20.18.0 和 Python 3.12。
...
它只跳过 Windows EXE 图标/资源编辑，不跳过应用文件、后端嵌入、目录包内容和内置后端验证。当前 Windows CI 同样设置 `REPOMIND_SKIP_WINDOWS_EXE_METADATA=1`，用于避免网络受限环境下载 `winCodeSign`；正式 Tag Release workflow 不使用该跳过开关，仍按标准 Release 配置构建 NSIS/Portable 产物。
...
- `public-main`、`main`、`master` 的 push；
- pull request；
- 手动触发。
...
| 后端测试 | PASS（本地） | 122 passed，60 warnings；不是远端 CI 结果 |
| 桌面测试 | PASS（本地） | 53 passed，6 个测试文件；不是远端 CI 结果 |
| Renderer/Electron build | PASS | production build 与 tsc 通过 |
| 正式 PyInstaller EXE | PASS（历史本地构建） | formal spec 曾构建成功；不是当前 Release 附件 |
| 源冻结后端 smoke | PASS（历史本地构建） | Schema/FTS5/no-Key/数据库身份/Session 通过 |
| Electron `win-unpacked` | PASS（历史本地构建） | 目录包曾生成成功 |
| 内置后端一致性 | PASS（历史本地构建） | 源/内置 SHA-256 相同 |
| 内置后端运行 | PASS（历史本地构建） | 最终 embedded smoke 通过 |
| 后端进程退出 | PASS（历史本地构建） | 认证关闭后无残留并验证文件锁释放；不使用强杀或名称扫描 |
| NSIS Setup | PASS（历史本地构建） | 曾生成 `RepoMind-0.1.0-x64-setup.exe`；不是当前可下载 Release 文件 |
| Portable | PASS（历史本地构建） | 曾生成 `RepoMind-0.1.0-x64-portable.exe`；不是当前可下载 Release 文件 |
| SHA-256 清单 | PASS（历史本地构建） | 曾生成 `SHA256SUMS.txt`；仅对应历史本地产物 |
...
启用 Chat 或 Embedding Provider 后，RepoMind 会把当前请求检索出的仓库 Evidence 发送到用户配置的 Base URL；内容可能包括源码、文件路径、配置片段、符号和用户问题，对应 API Key 也会按 Provider 接口要求发送。任意自定义 Endpoint 都是本地用户主动选择的信任边界，不是 RepoMind 认可或隔离的服务。只应使用可信 HTTPS Endpoint，并在分析私有仓库前确认服务方的日志、留存和访问策略。RepoMind 不提供任意 Provider URL 的网络沙箱、目的地址白名单或数据防泄漏保证。
...
- 不把本地构建产物写成 GitHub Release；当前 Release workflow 仅说明发布机制，不能替代实际 Release 页面。
- 不把静态图谱描述为运行时真实调用链。
