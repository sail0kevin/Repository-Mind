...
当前版本把 Specialist Tool 的依赖、测试和安全发现保存到 Agent Trace，用于解释本次回答过程，但不会修改不可变 Snapshot 中的 Evidence。未来只有在发现经过独立验证、具备稳定来源和迁移策略时，才考虑将其写入新的派生 Evidence 体系。
...
- 当前规则 Router 每次选择 0 或 1 个工具；执行器仍以最多两个工具作为硬上限，避免无限循环和无预算探索；
...
本文件是 `2026-07-28_ARCHITECTURE_FUTURE_ROADMAP_未来架构路线图.md` 的补充说明，用于统一面试和文档口径。具体实现细节、里程碑和验收标准以路线图为准。
